...
var SafeSticky = class extends HTMLElement {
  constructor() {
    ...
    __privateAdd(this, _initialTop, 100);
...
var Drawer = class extends Modal {
  ...
  createEnterAnimationControls() {
    ...
    return timeline3([
      ...
      [this.getShadowPartByName("content"), { opacity: [0, 1], transform: [translateOrigin, "translate(0)"] }, { duration: 0.3, at: "<", easing: [0.645, 0.045, 0.355, 1] }]
    ]);
  }
  createLeaveAnimationControls() {
    let translateOrigin = `${this.openFrom === "right" ? "translateX(20px)" : "translateX(-20px)"}`;
    ...
    return timeline3([
      ...
      [this.getShadowPartByName("content"), { opacity: [1, 0], transform: ["translate(0)", translateOrigin] }, { duration: 0.3, at: "<", easing: [0.645, 0.045, 0.355, 1] }]
    ]);
  }
};
...
var QuickBuyModal = class extends Modal {
  ...
  async show() {
    ...
    Shopify?.PaymentButton?.init();
      ...
      const selectSwatchPicker = document.querySelectorAll('.variant-picker__option--swatch');
      selectSwatchPicker.forEach(picker => {
        const toggle = picker.querySelector('.block-swatch--selected');
        const dropdown = picker.querySelector('.variant-picker__option-dropdown');
...
onSummaryClicked_fn = function(event) {
  ...
  this.toggle();
  this.parentElement.querySelector(".overlay_mega_menu").addEventListener("click", function(){
    this.parentElement.parentElement.toggle();
    this.parentElement.querySelector(".overlay_mega_menu").style.height = "0";
  })
  this.parentElement.querySelector(".overlay_mega_menu").style.height = "calc(50vh)";
};
...
detectClickOutside_fn = function(event) {
  ...
  if (event.target.getAttribute('href') || event.target.closest('a') && event.target.closest('a').getAttribute('href')) {
    this.toggle(false);
  }
};
/**
...
doPredictiveSearch_fn = async function() {
  ...
  document.documentElement.dispatchEvent(new CustomEvent("theme:loading:end", { bubbles: true }));
  if (tempDoc.querySelector('.predictive-search__no-results')) {
    document.querySelector('.header-search__default').style.display = 'flex';
  } else {
    document.querySelector('.header-search__default').style.display = 'none';
  }
};
/**
...
reveal_fn2 = async function() {
  ...
  const imageTransform = hasParallax ? [`scale(1.5) translateY(-${transformEffect}%)`, `scale(1.4) translateY(-${transformEffect}%)`] : ["scale(1.3)", "scale(1)"];
  return timeline8([
...
onFormVisibilityChange_fn = function(entries) {
  ...
  this.classList.toggle("is-visible", __privateGet(this, _latestFormCondition) && (__privateGet(this, _footerElement) ? __privateGet(this, _latestFooterCondition) : true));
};
...
var _delegate8, _onVideoEndedListener, _SlideshowCarousel_instances, autoplayPauseOnVideo_get, getSlideEnteringSequence_fn, getSlideLeavingSequence_fn, muteVideo_fn, unmuteVideo_fn, onVolumeChange_fn, playVideo_fn,pauseVideo_fn,onPlayPauseChange_fn,onSlideSettle_fn, onVideoEnded_fn, onNextButtonClicked_fn, handleAutoplayProgress_fn;
var SlideshowCarousel = class extends EffectCarousel {
  constructor() {
    ...
    __privateGet(this, _delegate8).on("click", '[data-action="play"]', __privateMethod(this, _SlideshowCarousel_instances, playVideo_fn).bind(this));
    __privateGet(this, _delegate8).on("click", '[data-action="pause"]', __privateMethod(this, _SlideshowCarousel_instances, pauseVideo_fn).bind(this));
    
    this.addEventListener("volumechange", __privateMethod(this, _SlideshowCarousel_instances, onVolumeChange_fn), { capture: true });
    this.addEventListener("play", __privateMethod(this, _SlideshowCarousel_instances, onPlayPauseChange_fn), { capture: true });
    this.addEventListener("pause", __privateMethod(this, _SlideshowCarousel_instances, onPlayPauseChange_fn), { capture: true });
...
getSlideEnteringSequence_fn = function(slide) {
  ...
  if (slideContent.classList.contains("slideshow__slide-content--boxed")) {
    return [
      ...
      [slide.querySelectorAll(".content-over-media > :is(video-media, svg), .content-over-media > picture img"), { opacity: [0, 1], transform: ["scale(1.3)", "scale(1)"] }, { duration: 0.8, at: "<", easing: [0.25, 0.46, 0.45, 0.94] }],
...
/**
 * Play the video for the active slide, by ignoring the video that may not be visible
 */
playVideo_fn = function(event) {
  event.preventDefault();
  event.stopPropagation();
  Array.from(this.selectedCell.querySelectorAll("video")).filter((video) => video.offsetParent).forEach((video) => video.play());
};
/**
 * Pause the video for the active slide, by ignoring the video that may not be visible
 */
pauseVideo_fn = function(event) {
  event.preventDefault();
  event.stopPropagation();
  Array.from(this.selectedCell.querySelectorAll("video")).filter((video) => video.offsetParent).forEach((video) => video.pause());
};
/**
...
/**
 * Update play pause controls button (if any) based on whether the video is play or not
 */
onPlayPauseChange_fn = function(event) {
  const playPauseControl = event.target.closest(".slideshow__slide").querySelector(".slideshow__play_pause-control");
  if (playPauseControl) {
    playPauseControl.querySelector('[data-action="play"]').hidden = !event.target.paused;
    playPauseControl.querySelector('[data-action="pause"]').hidden = event.target.paused;
  }
};
...
(() => {
  ...
  const customerSectionsToggle = document.querySelectorAll('[toggle-customer-section]');
  const customerSections = document.querySelectorAll('.customer-account__content');
  const customerNav = document.querySelectorAll('.customer-account__sidebar .cta');
  if (customerSectionsToggle) {
    customerSectionsToggle.forEach(toggle => {
      toggle.addEventListener('click', function() {
        const target = toggle.getAttribute('toggle-customer-section');
        ...
        if (toggle.classList.contains('cta')) {
          customerNav.forEach(link => {
            link.classList.remove('cta--underline');
          });
          toggle.classList.add('cta--underline');
        }
      });
    });

    if (window.location.hash === '#orders') {
      const orders = document.getElementById('customer-orders');
      if (orders) {
        customerSections.forEach(section => {
          section.setAttribute('hidden', true);
        });
        ...
        document.querySelector('.cta[href="#orders"]').classList.add('cta--underline');
      }
    }
  }

  document.querySelectorAll('.carousel-dots').forEach(dotsContainer => {
    const carouselId = dotsContainer.getAttribute('data-dots-for');
    const carousel = document.getElementById(carouselId);
    if (!carousel) return;

    const cells = carousel.cells || Array.from(carousel.children);
    ...
    cells.forEach((cell, index) => {
      ...
      if (index === (carousel.selectedIndex || 0)) {
    ...
    const updateActiveDot = (index) => {
      ...
      buttons.forEach((btn, i) => {
        ...
        btn.classList.toggle("is-near", Math.abs(i - index) === 1 || Math.abs(i - index) === 2);
      });
    };
  ...
  document.querySelectorAll('.product-gallery__progress').forEach(wrapper => {
    const carouselId = wrapper.getAttribute('data-progress-for');
    const bar = wrapper.querySelector('.product-gallery__progress-bar');
    const carousel = document.getElementById(carouselId);
  ...
  const mobileLinks = document.querySelectorAll('.header-sidebar__linklist-button.last-item, .header-sidebar__footer [href="/search"]');
  if (mobileLinks.length > 0) {
    mobileLinks.forEach(link => {
      link.addEventListener('click', function() {
        document.querySelector('header-sidebar').dispatchEvent(new CustomEvent("dialog:force-close", { bubbles: true, cancelable: true, composed: true }));
      });
    });
  }

  const tocList = document.querySelector(".article-summary__list");
  const articleContent = document.querySelector("[class*=template-article] #main");

  if (tocList && articleContent) {
    const headings = articleContent.querySelectorAll("h2");

    if (!headings.length) {
      document.querySelector(".article-summary__wrapper")?.remove();
      return;
    }
  ...
  if (NotifCenter) {
    document.getElementById('notifications-trigger-wrapper')?.style.setProperty('display', '');
    ...
    const badge = trigger?.querySelector(".notifications-badge");
    ...
    function addToPanel(toast) {
      if (!panelList.querySelector(`[data-id="${toast.dataset.id}"]`)) {
        const clone = toast.cloneNode(true);
        clone.classList.remove('hidden');
        clone.classList.add("notification-panel-item");
    ...
    toasts.forEach((toast) => {
      ...
      const delay = parseInt(toast.dataset.delay, 10);
      const duration = parseInt(toast.dataset.duration, 10);
      const firstOnly = toast.dataset.firstVisit === "true";
      const returningOnly = toast.dataset.returning === "true";
      const alreadySeen = shownNotifications.includes(id);
      ...
      if (firstOnly && !isFirstVisit) return;
      if (returningOnly && isFirstVisit) return;
    ...
    panel?.querySelector(".notifications-panel__close")?.addEventListener("click", () => {
  ...
  function isFreeGiftActive() {
    const config = window.freeGiftConfig;
    
    if (!config || !config.enabled || !config.variantId) return false;
  ...
  let isSyncingFreeGift = false;

  async function getCart() {
    return fetch('/cart.js').then(res => res.json());
  }

  function hasPaidProduct(cart) {
  ...
  function getInvalidManualFreeGifts(cart) {
    const variantId = window.freeGiftConfig?.variantId;
    return cart.items.filter(
      item => item.id === variantId && !item.properties?.[FREE_GIFT_PROPERTY]
    );
  }
  
  async function addFreeGift() {
    const { variantId } = window.freeGiftConfig;
    const formData = new FormData();
    formData.append('id', variantId);
    formData.append('quantity', 1);
    formData.append(`properties[${FREE_GIFT_PROPERTY}]`, 'true');
  ...
  async function removeFreeGift(cart) {
    const giftItem = cart.items.find(item => item.properties?.[FREE_GIFT_PROPERTY]);
    if (!giftItem) return;
  ...
  async function syncFreeGift(cart) {
    if (isSyncingFreeGift) return;
    isSyncingFreeGift = true;

    const active = isFreeGiftActive();
    const hasGift = hasFreeGift(cart);

    // Si la fonctionnalité est désactivée ou hors période, on retire le cadeau s'il est présent
    if (!active) {
      if (hasGift) {
        await removeFreeGift(cart);
        document.dispatchEvent(new CustomEvent('cart:refresh'));
      }
      isSyncingFreeGift = false;
      return;
    }
    ...
    const invalidManualGifts = getInvalidManualFreeGifts(cart);
    if (invalidManualGifts.length) {
      await fetch('/cart/change.js', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: invalidManualGifts[0].key, quantity: 0 })
      });
      document.dispatchEvent(new CustomEvent('cart:refresh'));
    }
